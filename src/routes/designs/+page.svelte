...
<svelte:head>
	<title>Designs | Aidan Neel</title>
	<meta
		name="description"
...
<div class="buffer">
	<section aria-labelledby="designs-title">
		<div class="ln designs-nav">
			<a href={resolve('/')}>aidan-neel</a>
			<a href={resolve('/designs')} aria-current="page">designs</a>
		</div>
		<div class="ln"></div>
		<div class="ln comment">
			<h1 id="designs-title"># visual - designs</h1>
		</div>
		<div class="ln comment">
			<p>{designs.length} pieces · {withSound} play a song</p>
		</div>
		<div class="ln"></div>
		<div class="moodboard">
			{#each designs as design, index (design.image)}
				<button
					type="button"
					class="design-tile"
					style={`--enter-delay: ${Math.min(index, 14) * 50 + 160}ms`}
				...
				>
					...
					<span class="design-caption">
						...
						<span class="dim design-date">{design.date}</span>
					</span>
				</button>
			{/each}
		</div>
	...
	<footer>
		<div class="ln"></div>
		<div class="ln comment">
			<p># aidan neel - {new Date().getFullYear()}</p>
		</div>
		<div class="ln comment">
			<p># eof</p>
		</div>
